...
abstract class AbsCameraActivity<T : ViewBinding> : CommonActivity<T>() {
    ...
    open fun initSwitchCamera() = false

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        isFront = initSwitchCamera()
        val permissions = arrayListOf(
            Permission.READ_MEDIA_IMAGES,
            Permission.CAMERA,
            Permission.WRITE_EXTERNAL_STORAGE
        )
        if (XXPermissions.isGranted(this, permissions)) startCamera(isFront)
    ...
    open fun takePhoto() {
        ...
        showProgressDialog(R.string.taking_photo)
        imageCapture.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                override fun onError(exc: ImageCaptureException) {
                    ...
                    onTakePictureError(exc)
                }

                override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                    LogKit.d(javaClass.simpleName, "take Photo success")
                    onImageSaved(output.savedUri ?: return)
                }
            })
        //floatAnimate(0L, 400L, 0f, 1f, 1f, 0f) { binding.flashCover.alpha = it }
    }

    open fun onTakePictureError(exc: ImageCaptureException) {
        showErrorToast(exc.message ?: "")
    }

    open fun onImageSaved(uri: Uri) {
        setResult(RESULT_OK, Intent().also { it.putExtra(Constants.Params.ARG1, uri.toString()) })
        finish()
    }
